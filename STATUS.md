...
- 截止：2026-08-21 / v0.8.0
- 当前实际阶段：维护
- 当前需求基线：`docs/00-prd/PRD.md`（v0.8 已验收）
...
- Chrome/Edge 共用 MV3 扩展包与 v0.8.0 GitHub Release（TASK-009）。
...
- 多张本地封面会占用当前油猴或扩展的浏览器本地存储，因此必须统一裁切压缩并限制体积。
- `/albums` 集成依赖机核 React hydration 时机；当前 1.5 秒保护已实测通过，极慢首屏仍是低风险兼容点。
- Edge 按用户要求不做真机测试；仅验证同一 MV3 包未使用 Edge 不支持的专属 API。

## 下一步

- 维护 Chrome/Edge MV3、机核 DOM 与存储桥兼容性。
...
- [TASK-009](docs/04-tasks/TASK-009-chromium-extension-release.md)
...
- Chromium 扩展实测：Chrome 151 首次为空播单；创建后刷新仍保留，第二标签读取同一数据；无 GCPL 控制台错误。Edge 按用户要求未实测。
- 扩展构建实测：唯一业务源码、最小权限、存储桥全部竞态测试、两文件 ZIP 白名单和跨三时区确定性 SHA-256 通过；v0.8.0 Release 资产与远端哈希已验证。
