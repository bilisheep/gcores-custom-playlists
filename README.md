...
- 创建多个播单，并修改名称、删除播单。
- 在机核官方“播单”页面优先展示“我的”，以同类卡片和完整详情管理本地播单。
- 为本地播单选择自定义图片封面；图片只在浏览器内裁切压缩并保存在本机。
- 从节目卡片右上角的“列表＋”按钮加入当前播单。
- 在播单内播放、移除节目以及调整顺序。
- 使用底部居中的磨砂玻璃播放器控制播放/暂停、下一期、可拖动进度和弹出式竖直音量。
- 每个播单分别保存当前节目和播放时间，下次从断点继续。
- 在用户页一键加入该用户作为 DJ/主播参与的全部公开节目，自动遍历分页并去重。
- 在音频专辑页一键加入全部已发布节目，支持免费和付费音频专辑。
- 每个播单可切换正序/倒序播放，上一期、下一期和自然播完均遵循当前方向。
...
![完整播单详情和节目管理](docs/screenshots/playlist-manager.jpg)

### 官方播单页中的“我的”

![官方播单页中的本地播单与自定义封面](docs/screenshots/my-playlists-page.jpg)

### 音频专辑一键加入

![音频专辑一键加入当前播单](docs/screenshots/album-batch-add.jpg)
...
截图来自真实 `gcores.com` 页面和脚本实际运行状态，登录头像及通知数量已隐藏。截图对应版本：v0.7.0。截图中的 GCORES 商标、页面界面、节目名称、封面和公开用户资料归各自权利人所有，不属于本项目 MIT 授权范围。
...
4. 点击封面右侧的标题或播单信息，进入所属本地播单的完整详情页。
5. 使用顶部的 `＋`、`修改名字`、`删除播单` 管理播单。
6. 使用“排序：正序/倒序”切换播放方向；节目列表、移动按钮和下一期会同步反向。
7. 使用节目右侧按钮播放、上移、下移或移除节目。

### 我的播单页面与封面

1. 点击机核左侧的官方“播单”，无筛选参数时默认进入顶部首位的“我的”。
2. “我的”使用与机核播单相同的响应式卡片网格；“自建”角标用于区分本地数据。
3. 点击卡片进入完整播单详情；“机核 / 入驻”继续显示原有官方内容和分页。
4. 点击卡片右上角相机按钮，或详情中的“设置封面 / 更换封面”，选择本地图片。
5. 图片会居中裁成 320×320 JPEG；可用“移除封面”恢复首期节目封面或音乐占位图。

### 播放和断点续播

- 底部播放器主行控制播放/暂停和下一期；第二行显示当前时间、可拖动进度和总时长。
- 点击右侧音量图标会在上方展开竖直滑杆，向上增大、向下减小；点击外部或按 Esc 关闭，也可使用方向键、Home 和 End。
- 完整播单详情页中的“从断点播放”会恢复该播单上次停止的节目和时间。
...
1. 打开机核音频专辑页，例如 [`/albums/230`](https://www.gcores.com/albums/230)。
2. 在“播放全部”旁点击“加入当前播单”。
3. 脚本会按机核官方顺序遍历所有分页，并在全部读取成功后一次加入。
4. 已存在节目会自动跳过；付费节目可加入，但播放仍取决于当前机核账号权限。

“正序”表示播单当前规范/手动顺序，“倒序”表示其反向播放。切换方向不会改变正在播放的节目或断点；二维码按当前可见顺序分享，导入后的新播单默认正序。
